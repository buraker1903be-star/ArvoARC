-- Kategorileme kurumu çağırandan alsın.
--
-- Fonksiyon organizasyonu slug = 'arvoculture' diye sabit seçiyordu: ikinci
-- mağaza açıldığında onun ürünleri hiç kategorilenmezdi. Artık kurum parametre;
-- verilmezse eski davranışa düşer, böylece elle yapılan çağrılar bozulmaz.
--
-- Cron da değişti: her çalıştırmada tek mağaza işleniyor (yarım kalan önce,
-- yoksa en uzun süredir güncellenmeyen). Tüm mağazaları tek istekte dolaşmak
-- cron süresini mağaza sayısıyla büyütürdü.

-- Parametre eklemek create or replace ile OLMAZ: imza değiştiği için eskisinin
-- yanına ikinci bir sürüm eklenir ve tek argümanlı çağrı "function is not unique"
-- hatası verir. Eski imza önce düşürülüyor.
drop function if exists public.arc_categorize_supplier_products(text);

create or replace function public.arc_categorize_supplier_products(
  p_supplier text default 'tarzyeri'::text,
