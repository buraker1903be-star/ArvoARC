...
export default async function OrderDetail({params,searchParams}:{params:Promise<{id:string}>;searchParams:Promise<{saved?:string;error?:string}>}){
  ...
  return <Shell active="orders" tenantName={organization.name} tenantPlan={organization.plan_code}>
    <section className="subhead"><div><small><Link href="/siparisler">SİPARİŞLER</Link> · {sourceLabel(order.source)}</small><h2>{order.order_number}</h2><p>{new Date(order.created_at).toLocaleString("tr-TR")} · {order.customer_name||order.customer_email||"Misafir müşteri"}</p></div></section>
    ...
    <div style={{display:"grid",gridTemplateColumns:"minmax(0,1.5fr) minmax(280px,.7fr)",gap:24}}>
      <section className="card" style={{padding:24}}><div className="head"><div><small>SİPARİŞ KALEMLERİ</small><h3>{items?.length??0} kalem</h3></div><span>{sourceLabel(order.source)}</span></div>
      ...
      <aside className="card" style={{padding:24}}><div className="head"><div><small>DURUM</small><h3>Yönetim</h3></div></div>
        {canManage?<form action={updateOrderStatus} style={{display:"grid",gap:14,marginTop:16}}><input type="hidden" name="order_id" value={order.id}/><label>Sipariş durumu<select name="status" defaultValue={order.status} style={{display:"block",width:"100%",padding:12,marginTop:6}}>{orderStatusOptions.map(([value,label])=><option key={value} value={value}>{label}</option>)}</select></label><label>Ödeme durumu<select name="payment_status" defaultValue={order.payment_status} style={{display:"block",width:"100%",padding:12,marginTop:6}}>{paymentStatusOptions.map(([value,label])=><option key={value} value={value}>{label}</option>)}</select></label><button type="submit" style={{padding:12}}>Durumu kaydet</button></form>:<p>{orderStatusLabel(order.status)} · {paymentStatusLabel(order.payment_status)}</p>}
    ...
    <section className="card" style={{padding:24,marginTop:24}}><div className="head"><div><small>İŞLEM GEÇMİŞİ</small><h3>Zaman çizelgesi</h3></div><span>{events?.length??0} kayıt</span></div><div style={{marginTop:16}}>{events?.length?events.map(event=>{const data=(event.event_data??{}) as Record<string,string|null>;const title=event.event_type==="status_updated"?"Sipariş durumu güncellendi":"Kargo bilgileri güncellendi";const detail=event.event_type==="status_updated"?`${orderStatusLabel(data.old_status)} → ${orderStatusLabel(data.new_status)} · Ödeme: ${paymentStatusLabel(data.new_payment_status)}`:`${data.shipping_carrier||"Kargo firması yok"} · ${data.tracking_number||"Takip numarası yok"}`;return <div className="order" key={event.id}><i>✓</i><div><b>{title}</b><small>{detail}</small></div><span style={{textAlign:"right",fontSize:10}}>{new Date(event.created_at).toLocaleString("tr-TR")}<small style={{display:"block"}}>{event.created_by?"Yetkili kullanıcı":"Sistem"}</small></span></div>}):<p>Henüz kayıtlı sipariş işlemi yok.</p>}</div></section>
