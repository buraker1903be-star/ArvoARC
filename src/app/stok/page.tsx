...
export default async function Stock({ searchParams }: { searchParams: Promise<{ error?: string; updated?: string }> }) {
  ...
  return <Shell active="stock" tenantName={organization.name} tenantPlan={organization.plan_code}>
    ...
    <section className="card table"><div className="head"><div><small>STOK</small><h3>{variants?.length ?? 0} varyant</h3></div><span>{organization.name}</span></div><div className="row th"><span>ÜRÜN</span><span>SKU</span><span>STOK</span><span>POLİTİKA</span><span>DURUM</span></div>{(variants ?? []).map((variant,index)=><div className="row" key={variant.id}><span><i className={`swatch s${index%5}`}>AC</i><b>{productName.get(variant.product_id) ?? "Ürün"}</b></span><span>{variant.sku}</span><span>{variant.stock}</span><span>{variant.allow_backorder ? "Stoksuz satış açık" : "Stok zorunlu"}</span><span><em>{variant.stock < 0 ? "Tedarik gerekli" : variant.stock === 0 ? "Stok sıfır" : "Stokta"}</em></span></div>)}</section>
