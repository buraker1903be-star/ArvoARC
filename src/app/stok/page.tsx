...
export default async function Stock({ searchParams }: { searchParams: Promise<{ error?: string; updated?: string; q?: string; filter?: string }> }) {
  ...
  const [{ data: variants, error: variantsError }, { data: products, error: productsError }, { data: movements, error: movementsError }] = await Promise.all([
  ...
  const low=(variants??[]).filter(variant=>variant.stock>0&&variant.stock<=5);
  ...
  const visibleVariants=(variants??[]).filter(variant=>{
    ...
    if(stockFilter==="low")return variant.stock>0&&variant.stock<=5;
  ...
  return <Shell active="stock" tenantName={organization.name} tenantPlan={organization.plan_code}>
    ...
    <section className="metrics"><article><span>TOPLAM STOK</span><strong>{totalUnits}</strong><small>{variants?.length??0} varyant</small></article><article><span>NEGATİF</span><strong>{negative.length}</strong><small>Tedarik gerekli</small></article><article><span>STOK SIFIR</span><strong>{zero.length}</strong><small>Satış politikası kontrolü</small></article><article><span>DÜŞÜK STOK</span><strong>{low.length}</strong><small>1–5 adet arası</small></article></section>
