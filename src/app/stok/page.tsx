  ...
  */
  ...
  return <Shell active="stock" tenantName={organization.name} tenantPlan={organization.plan_code}>
    <section className="ac-bar">
      <div>
        <h1>Stok Yönetimi</h1>
        <p>Tüm değişiklikler hareket bazında kaydedilir.</p>
      </div>
      <div className="ac-bar-actions">
        <a className="ac-btn" href="/api/disari-aktar/stok">CSV indir</a>
      </div>
    </section>

    <div className="ac-stack">
    {params.updated === "1" && <section className="ac ac-pad-sm"><strong>Stok hareketi kaydedildi.</strong></section>}
    {params.error && <section className="ac ac-pad-sm"><strong>İşlem tamamlanamadı: {params.error}</strong></section>}
    <section className="ac-metrics"><article className="ac-metric"><span>TOPLAM STOK</span><strong>{(typeof stockSum==="number"?stockSum:0).toLocaleString("tr-TR")}</strong><small>{(variantCount??0).toLocaleString("tr-TR")} varyant</small></article><article className="ac-metric"><span>NEGATİF</span><strong>{negative.length}</strong><small>Tedarik gerekli</small></article><article className="ac-metric"><span>STOK SIFIR</span><strong>{zero.length}</strong><small>Satış politikası kontrolü</small></article><article className="ac-metric"><span>DÜŞÜK STOK</span><strong>{low.length}</strong><small>1–{lowStockThreshold} adet arası</small></article></section>
    <section className="ac ac-pad-sm"><form className="ac-filter"><input name="q" defaultValue={params.q??""} placeholder="SKU ile ara"/><select name="filter" defaultValue={stockFilter}><option value="all">Tüm varyantlar</option><option value="negative">Negatif stok</option><option value="zero">Stok sıfır</option><option value="low">Düşük stok (1–5)</option><option value="available">Stokta var</option><option value="backorder">Stoksuz satış açık</option></select><button className="ac-btn ac-btn-primary" type="submit">Filtrele</button>{(params.q||stockFilter!=="all")&&<a className="ac-btn" href="/stok">Temizle</a>}</form></section>
    ...
    {canManage&&<section className="ac ac-pad"><div className="ac-head"><div><h3>Stok güncelle</h3><p>Her değişiklik hareket olarak kaydedilir.</p></div></div><form action={adjustInventory} style={{display:"grid",gridTemplateColumns:"2fr 1fr 1fr",gap:12,marginTop:18}}>
      <label>Varyant<select name="variant_id" required className="ac-input"><option value="">Seçin</option>{(variants ?? []).map((variant)=><option key={variant.id} value={variant.id}>{productName.get(variant.product_id) ?? "Ürün"} · {variant.sku} · stok {variant.stock}{variant.allow_backorder ? " · stoksuz satış açık" : ""}</option>)}</select></label>
