...
export default async function Analytics(){
  ...
  const now=new Date();const months=Array.from({length:6},(_,index)=>{const date=new Date(Date.UTC(now.getUTCFullYear(),now.getUTCMonth()-(5-index),1));return {key:`${date.getUTCFullYear()}-${date.getUTCMonth()}`,date,revenue:0,orders:0};});
  ...
  return <Shell active="analytics" tenantName={organization.name} tenantPlan={organization.plan_code}>
    ...
    <section className="grid">
      ...
      <article className="card"><div className="head"><div><small>DAĞILIM</small><h3>Sipariş durumları</h3></div><span>{allOrders.length} toplam</span></div><div style={{display:"grid",gap:12,marginTop:20}}>{Object.entries(statusLabels).map(([status,label])=>{const count=statusCounts.get(status)??0;return <div key={status}><span style={{display:"flex",justifyContent:"space-between",fontSize:12}}><b>{label}</b><strong>{count}</strong></span><div style={{height:8,background:"#eef0eb",marginTop:7}}><i style={{display:"block",height:"100%",width:`${allOrders.length?count/allOrders.length*100:0}%`,background:"var(--green)"}}/></div></div>})}</div></article>
    </section>
