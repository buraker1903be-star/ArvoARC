  ...
  return <Shell active="collections" tenantName={organization.name} tenantPlan={organization.plan_code}>
    ...
    <section className="collection-grid">
      {(collections??[]).map((collection,index)=><Link href={`/koleksiyonlar/${collection.id}`} className="collection-card" key={collection.id}>
        <div className={`collection-cover tone-${index%5}`}><span>{collection.source==="shopify"?"SHOPIFY EŞLEMESİ":"ARVO ARC"}</span><b>{collection.title.split(" ").map(word=>word[0]).join("").slice(0,3).toUpperCase()}</b><em>{statusLabels[collection.status]??collection.status}</em></div>
