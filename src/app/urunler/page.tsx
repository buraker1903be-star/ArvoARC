...
export default async function Products({ searchParams }: { searchParams: Promise<{ error?: string; created?: string }> }) {
  ...
  const [{ data: products, error: productsError }, { data: variants, error: variantsError }] = await Promise.all([
    ...
    supabase.from("arc_product_variants").select("id,product_id,sku,price,currency,stock,allow_backorder").eq("organization_id", organization.id),
  ]);
  ...
  return <Shell active="products" tenantName={organization.name} tenantPlan={organization.plan_code}>
    <section className="subhead"><div><small>KATALOG · CANLI</small><h2>Ürünler</h2><p>Stok takibi isteğe bağlıdır. Stoksuz satış açık ürünlerde stok sıfırın altına düşebilir ve satış devam eder.</p></div></section>
    ...
    {canManage && <section className="card" style={{padding:24,marginBottom:24}}><div className="head"><div><small>YENİ ÜRÜN</small><h3>Kataloğa ekle</h3></div><span>Native ARC ürün</span></div><form action={createProduct} style={{display:"grid",gridTemplateColumns:"repeat(2,minmax(0,1fr))",gap:14,marginTop:20}}>
      ...
      <label style={{display:"flex",alignItems:"center",gap:10}}><input name="allow_backorder" type="checkbox" defaultChecked /> Stok yokken satışa devam et</label>
    ...
    <section className="card table"><div className="head"><div><small>KATALOG</small><h3>{products?.length ?? 0} ürün</h3></div><span>{organization.name}</span></div><div className="row th"><span>ÜRÜN</span><span>SKU</span><span>STOK</span><span>FİYAT</span><span>DURUM</span></div>
      {products?.length ? products.map((product,index)=>{const variant=variants?.find(item=>item.product_id===product.id);const stockLabel=variant ? `${variant.stock}${variant.allow_backorder ? " · stoksuz satış açık" : ""}` : "—";return <div className="row" key={product.id}><span><i className={`swatch s${index%5}`}>AC</i><b>{product.name}</b></span><span>{variant?.sku??"—"}</span><span>{stockLabel}</span><span>{variant?money.format(variant.price/100):"—"}</span><span><em>{product.status}</em></span></div>}) : <div style={{padding:24}}><strong>Henüz ürün yok.</strong><p>Yukarıdaki formdan ilk ArvoARC ürününü oluşturabilirsiniz.</p></div>}
    </section>
  </Shell>;
}
